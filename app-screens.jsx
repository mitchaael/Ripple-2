...
function StatsScreen({ habits, meta }) {
  ...
  return (
    <div style={{flex:1,overflowY:'auto',overflowX:'hidden',padding:'calc(env(safe-area-inset-top,0px) + 18px) 16px 100px'}}>
...
function LogrosScreen({ habits, meta }) {
  ...
  return (
    <div style={{flex:1,overflowY:'auto',padding:'calc(env(safe-area-inset-top,0px) + 18px) 16px 100px',position:'relative'}}>
...
function SettingsScreen({ meta, onMetaChange, habits, onClear }) {
  ...
  const [showBackup, setShowBackup] = uS(false);
  const [todayMood, setTodayMood] = uS(()=>loadDayMood(todayStr()));
  ...
  return (
    <div style={{flex:1,overflowY:'auto',padding:'calc(env(safe-area-inset-top,0px) + 18px) 16px 100px'}}>
      ...
      {/* ── Mood del día ── */}
      <p style={{color:T.textDim,fontSize:10,fontWeight:600,textTransform:'uppercase',letterSpacing:'.08em',marginBottom:10}}>Estado de ánimo de hoy</p>
      <div style={card({padding:'14px 16px'})}>
        <div style={{display:'flex',gap:8,marginBottom:10}}>
          {[{id:'great',e:'🤩',l:'Genial'},{id:'good',e:'😊',l:'Bien'},{id:'ok',e:'😐',l:'Regular'},{id:'hard',e:'😔',l:'Difícil'},{id:'rough',e:'😩',l:'Duro'}].map(m=>(
            <button key={m.id} onClick={()=>{setTodayMood(m.id);saveDayMood(todayStr(),m.id);}} style={{flex:1,padding:'8px 4px',borderRadius:12,border:`1.5px solid ${todayMood===m.id?T.accent:T.cardBorder}`,background:todayMood===m.id?T.accentDim:T.chipInactive,cursor:'pointer',transition:'all .18s'}}>
              <div style={{fontSize:20}}>{m.e}</div>
              <div style={{color:todayMood===m.id?T.accentText:T.textDim,fontSize:9,fontWeight:600,marginTop:2}}>{m.l}</div>
            </button>
          ))}
        </div>
        <p style={{color:T.textDim,fontSize:11,marginTop:4}}>Historial de los últimos 7 días:</p>
        <div style={{marginTop:8}}><MoodHistoryWidget/></div>
      </div>

      {/* ── Vacation mode ── */}
      <p style={{color:T.textDim,fontSize:10,fontWeight:600,textTransform:'uppercase',letterSpacing:'.08em',marginBottom:10}}>Viajes</p>
      <div style={card({padding:'4px 18px'})}>
        <Row icon="🏖️" label="Modo vacaciones" right={<Toggle val={JSON.parse(localStorage.getItem('mh4_vacation')||'false')} onChange={()=>{const v=!JSON.parse(localStorage.getItem('mh4_vacation')||'false');localStorage.setItem('mh4_vacation',JSON.stringify(v));window.location.reload();}}/>}/>
      </div>

      {/* ── Backup ── */}
      <p style={{color:T.textDim,fontSize:10,fontWeight:600,textTransform:'uppercase',letterSpacing:'.08em',marginBottom:10}}>Backup</p>
      <div style={card({padding:'4px 18px'})}>
        <Row icon="💾" label="Backup & Restaurar" onClick={()=>setShowBackup(true)}
