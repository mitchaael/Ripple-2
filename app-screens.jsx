        ...
        <div className="overlay" onClick={e=>e.target===e.currentTarget&&setConfirmReset(false)}>
          <div className="sheet" style={{background:T.sheetBg,textAlign:'center'}}>
            ...
            <div style={{display:'flex',gap:10}}>
              ...
              <button onClick={()=>{
                // Borrar todo el localStorage de Ripple
                const keys = Object.keys(localStorage).filter(k=>k.startsWith('mh4_')||k.startsWith('mh4')||k==='mh4_lastReset');
                keys.forEach(k=>localStorage.removeItem(k));
                localStorage.removeItem('mh4_myday_tasks');
                localStorage.removeItem('mh4_myday_top');
                localStorage.removeItem('mh4_agenda_tasks');
                Object.keys(localStorage).filter(k=>k.includes('ripple')||k.includes('mh4')).forEach(k=>localStorage.removeItem(k));
                // Recargar la app
